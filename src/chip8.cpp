...
int Chip8::get_display_value(int i) { return display[i]; }
...
void Chip8::single_cycle(bool trace_mode, bool sound_on) {
  ...
  int opcode_msb_nibble = get_nibble(
      opcode, 12, 0xF000);  // if value is ABCD(each 4 bits), it returns A
  ...
  switch (opcode_msb_nibble) {
    ...
    case 8:
      ...
      switch (val) {
        ...
        case 2:
          // 8XY2. Sets VX to VX and VY. (Bitwise AND operation) VF is reset to
          // 0.
          reg1 = get_nibble(opcode, 8, 0x0F00);
          reg2 = get_nibble(opcode, 4, 0x00F0);
          V[reg1] &= V[reg2];
    ...
    case 13:  // 0xD
    {
      ...
      //  instruction. VF is set to 1 if any screen pixels are flipped from set
...
// helper functions
int Chip8::get_nibble(
    int val, int bits,
    int val_to_binary_and = 0xFFFF)  // extracts 4 bits from val
{
